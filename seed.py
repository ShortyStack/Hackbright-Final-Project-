...
def load_users():
    ...
    # Another for loop to iterate through
        ...
        user_id, email, password, street_address, zipcode = line_list[0], line_list[1], line_list[2], line_list[3], line_list[4]
        print "USER_ID %s, EMAIL: %s, PASSWORD: %s, STREET_ADDRESS: %s, ZIPCODE: %s" % (user_id, email, password, street_address, zipcode)

        user = User(user_id=user_id, email=email, password=password, street_address=street_address, zipcode=zipcode)
